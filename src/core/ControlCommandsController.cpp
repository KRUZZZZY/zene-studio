...
//! A template name must be a bare file name: it becomes one directory entry in
//! the user config dir, so a separator or a ".." would let a client write or
//! delete outside the template directory.
bool isSafeTemplateName(const QString& name, ControlResult* error)
{
	if (name.isEmpty())
	{
		*error = ControlResult::failure(ControlErrorKind::Refused,
...
void registerControllerTemplateSave(ControlRegistry& registry)
{
	...
	cmd.description = QStringLiteral("Save the project's current MIDI bindings as a named "
		"mapping template - one JSON file in the user config dir, listed by "
	...
	cmd.handler = [](const QJsonObject& args) {
		...
		ControlResult error;
		if (!isSafeTemplateName(name, &error)) { return error; }
...
void registerControllerTemplateList(ControlRegistry& registry)
{
	...
	cmd.description = QStringLiteral("Read-only: the mapping templates saved on this machine, "
		...
		"bindings it holds and whether every named control resolves in the CURRENT project. A "
		"template whose targets do not resolve still lists: it is a saved mapping set, not a "
		"claim about the project that is open.");
	cmd.argsSchema = objectSchema({});
	cmd.resultSchema = objectSchema({
		{QStringLiteral("count"), integerProperty(0, MaxSongLength)},
		{QStringLiteral("directory"), stringProperty()},
	...
	// A13: reads the user config dir, no display or device - swept headlessly.
	cmd.mutating = false;
	cmd.handler = [](const QJsonObject&) {
		ControllerSurface& surface = ControllerSurface::instance();
		const QStringList names = surface.listTemplates();

		// The resolvable set is read once: a template list is a report, not a
		// per-template walk of the song tree.
		QStringList resolvable;
		for (const BoundControl& bound : boundControls())
		{
			if (!bound.name.isEmpty()) { resolvable.append(bound.name); }
		}
		...
		for (const QString& name : names)
		{
			...
			{
				if (resolvable.contains(binding.targetName)) { ++resolvableCount; }
				else { missing.append(binding.targetName); }
			}
			QJsonObject entry;
			entry.insert(QStringLiteral("name"), name);
			entry.insert(QStringLiteral("bindings"), loaded.bindings.size());
			entry.insert(QStringLiteral("resolvable"), resolvableCount);
			entry.insert(QStringLiteral("missing"), missing);
...
void registerControllerTemplateApply(ControlRegistry& registry)
{
	...
	cmd.description = QStringLiteral("Apply a saved mapping template to this project: for every "
		...
		"another song applies the part that resolves and REPORTS the rest, rather than failing "
		"as a whole. A binding whose target is already driven by a MIDI controller is re-bound, "
		"which is what makes applying a template a way to move a surface onto a different set "
		"of addresses. This creates project state (the model's <connection> element), so the "
		"project is marked modified.");
	cmd.argsSchema = objectSchema({{QStringLiteral("name"), stringProperty()}});
	cmd.resultSchema = objectSchema({
		{QStringLiteral("name"), stringProperty()},
	...
	cmd.handler = [](const QJsonObject& args) {
		...
		if (!surface.listTemplates().contains(name))
		{
			return ControlResult::failure(ControlErrorKind::NotFound,
				QStringLiteral("no template named '%1' under %2: controller.template_list "
					"reports the names that exist").arg(name, ControllerSurface::templateDirectory()));
		}

		const int boundBefore = boundControls().size();
		// The surface does the binding; the command only reports it. ControllerSurface::
		// applyTemplate resolves each target by fullDisplayName() and skips what it
		// cannot resolve, which is the one implementation controller.template_apply, a
		// GUI menu and the tests all go through.
		const int created = surface.applyTemplate(name);

		QJsonArray controls;
		QStringList missing;
		for (const ControllerTemplateBinding& binding : surface.loadTemplate(name).bindings)
		{
		...
		for (const BoundControl& bound : boundControls())
		{
			if (!bound.name.isEmpty()) { controls.append(controlState(bound)); }
		}
		...
		result.insert(QStringLiteral("__transaction"),
			transactionPayload(
				QJsonObject{{QStringLiteral("name"), name},
					{QStringLiteral("bound_controls"), boundBefore}},
				QStringLiteral("controller.template_apply"),
				QJsonObject{{QStringLiteral("name"), name}},
				false,
				QStringLiteral("the created connections are serialized through their models' own "
					"<connection> elements, which no ProjectJournal checkpoint owns: there is no "
					"inverse command the engine can replay, so the recorded state is the binding "
					"list controller.surface_state reports")));
		return ControlResult::success(result);
	};
	registry.registerCommand(cmd);
}
...
void registerControllerTemplateDelete(ControlRegistry& registry)
{
	...
	cmd.description = QStringLiteral("Remove one saved mapping template from this machine. "
		"'removed' is false when no template of that name existed, which is reported rather "
		"than treated as an error (the file is the state, and the state asked for - no such "
		"file - already holds). No project state is touched: the project's own bindings are "
		"unaffected.");
	cmd.argsSchema = objectSchema({{QStringLiteral("name"), stringProperty()}});
	cmd.resultSchema = objectSchema({
		{QStringLiteral("name"), stringProperty()},
