 ...
 *                              vca.edit_move (SPEC A11-A16; the 0.3.0 ladder's
 *                              OWNER-31 item 11).
 *
 * The membership it acts on - vca.set_phase_lock, vca.track_add and
 * vca.track_remove - is the same half one step down, and lives in
 * ControlCommandsVcaEditSet.cpp. The two were one file until it reached 521
 * lines and tripped the 500-line file ratchet; the split is a MOVE (every
 * handler is byte-identical), and registerVcaEditCommands - below - is still
 * the group's single edit registration point.
 *
...
{
	// The edit SET and the lock switch are the group's other half, in their own
	// translation unit (see the file header): the membership and the switch, then
	// the phase-locked edit that uses both.
	registerVcaEditSetCommands(registry);
