...
//! Upstream #7323 removed MixHelpers::sanitize() and the NaN-handler toggle;
//! sanitization is now folded into the mix path. PROVISIONAL local equivalent
//! with the same semantics as the removed MixHelpers::sanitize().
auto sanitizeBuffer(SampleFrame* src, f_cnt_t frames) -> bool
{
	for (f_cnt_t f = 0; f < frames; ++f)
	{
		auto& frame = src[f];

		if (frame.containsInf() || frame.containsNaN())
		{
		...
		{
			frame.clamp(sample_t(-1000.0), sample_t(1000.0));
		}
	}
...
{
	// Upstream #7323 removed MixHelpers::useNaNHandler(); sanitization is now always on.
	...
	for (ch_cnt_t tc = 0; tc < numTrackChannels; tc += 2)
	{
		switch ((channels[tc] << 1) | std::uint8_t{channels[tc + 1]})
		{
			case 0b11:
			{
				...
				if (sanitizeBuffer(tcPair, m_frames))
				{
			...
			case 0b10:
			{
				if (sanitizeChannel(trackChannelPair(tc / 2), std::integral_constant<ch_cnt_t, 0>{}))
				{
					// Inf/NaN detected and buffer cleared
					m_quietChannels[tc] = true;
				}
			...
			case 0b01:
			{
				if (sanitizeChannel(trackChannelPair(tc / 2), std::integral_constant<ch_cnt_t, 1>{}))
				{
					// Inf/NaN detected and buffer cleared
					m_quietChannels[tc + 1] = true;
				}
...
{
	// m_channelPairs is the number of channel pairs, so iterate one pair at
	// a time (as sanitizeAll() does) rather than in track-channel steps
