...
struct WavInfo
{
	...
	qint64 dataSizeOffset = -1;  //!< file offset of the data chunk's 4 size bytes
...
WavInfo readWavInfo(const QString& path)
{
	...
	if (header.size() < 12 || !header.startsWith("RIFF") || header.mid(8, 4) != "WAVE") { return info; }
	...
	while (true)
	{
		...
		const quint32 size = qFromLittleEndian<quint32>(chunkHeader.constData() + 4);
		if (id == "fmt ")
		{
			...
			if (size % 2 == 1 && !file.seek(file.pos() + 1)) { break; }
		}
		else if (id == "data")
		{
			info.dataOffset = file.pos();
			info.dataSizeOffset = chunkStart + 4;
			info.declaredDataSize = static_cast<qint64>(size);
			break;
		}
		else if (!file.seek(chunkStart + 8 + size + (size % 2)))
		{
			break;
		}
	}
...
void registerProjectCommands(ControlRegistry& registry)
{
...

	{
		...
		cmd.handler = [](const QJsonObject& args) {
			const QString out = args.value(QStringLiteral("out")).toString();
			if (!out.startsWith(QLatin1Char('/')))
			{
				return ControlResult::failure(ControlErrorKind::InvalidArgs,
					QStringLiteral("'out' must be an absolute path"));
			}
			const QString formatName = args.value(QStringLiteral("format")).toString(QStringLiteral("wav"));
			ProjectRenderer::ExportFileFormat format = ProjectRenderer::ExportFileFormat::Wave;
			if (!formatFromName(formatName, &format))
			{
				return ControlResult::failure(ControlErrorKind::InvalidArgs,
					QStringLiteral("unsupported format '%1'").arg(formatName));
			}
			...
			// (`lmms render <file> -o <out>`), in a child process. Rendering
			// in-process would drive this instance's audio engine
			// (ProjectRenderer calls audioEngine()->startProcessing() /
			// stopProcessing(), which owns the device thread) and that leaves the
			// running instance's event loop unable to quit cleanly. The session is
			// serialised first, so the render is the audio truth of exactly the
			// state an agent has built.
			...
			renderer.start(QCoreApplication::applicationFilePath(),
				{QStringLiteral("render"), tempProject, QStringLiteral("-o"), out,
			...
			const int exitCode = finished ? renderer.exitCode() : -1;
			...
			{
				return ControlResult::failure(ControlErrorKind::Refused,
					QStringLiteral("the render failed (exit %1); the session is unchanged")
						.arg(finished ? exitCode : -1));
			}
			...
			const qint64 frames = format == ProjectRenderer::ExportFileFormat::Wave ? wavFrameCount(out) : -1;
