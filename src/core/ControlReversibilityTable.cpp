...
const ReversibilityRow* reversibilityRowTable(int* rowCount)
{
	static const std::vector<ReversibilityRow> joined = [] {
		...
		for (const ReversibilityRow* (*rowsFor)(int*) : {reversibilityActionRowTable,
				...
				reversibilityVerbRowTable, reversibilityScanAndCrashRowTable, reversibilityMasteringRowTable, reversibilityNoteScaleRowTable, reversibilityMeterRowTable, reversibilityExportPresetRowTable, reversibilityRecordingRowTable, reversibilityInterchangeRowTable, reversibilityChordRowTable})
		{
			int count = 0;
