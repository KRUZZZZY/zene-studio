#!/usr/bin/env python3
"""Merge-train 3A manifest regenerator/verifier.
...
merge train 3A #1).  `--rev HEAD` reproduces the header's command exactly (it is
...
def fork_cmd(rev):
    ...
    return (f"{{ {d} -- src include plugins tests/src/core tests/src/tracks "
            ...
            f"{d} -- tools/local-ci.sh tools/ncpu-shim.c ; }} | LC_ALL=C sort")
