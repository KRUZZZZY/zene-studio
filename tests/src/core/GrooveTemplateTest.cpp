...
private slots:
	...
	void theGeometryBoundsAreTheEnginesOwn()
	{
		...
		// note would belong to a different slot, which is a rearrangement.
	...
	void extractionReadsTheMeanDeviationPerSlot()
	{
		...
		// Notes at 9 / 26 / 34 / 51 land in slots 1 / 2 / 3 / 0 (modulo 4).
		QCOMPARE(captured.step(0), (GrooveStep{3, 0}));
		QCOMPARE(captured.step(1), (GrooveStep{-3, 20}));
		QCOMPARE(captured.step(2), (GrooveStep{2, -20}));
		QCOMPARE(captured.step(3), (GrooveStep{-2, 0}));
		...
		QCOMPARE(twoSlots.step(1), (GrooveStep{0, 0}));
		QCOMPARE(twoSlots.step(0), (GrooveStep{0, 0}));
	...
	void applicationIsExactAndIdempotent()
	{
		...
		// A velocity offset is clamped to the engine's own 0..200, so a groove
		// carried onto a quiet clip cannot push a note out of range.
		...
		QCOMPARE(static_cast<int>(quiet.vector()[0]->getVolume()), static_cast<int>(MaxVolume));
	}
