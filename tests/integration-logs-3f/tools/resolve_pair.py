...
# 3F NOTE: this resolver reads the three index stages (:1: base, :2: ours, :3: theirs).
# It DESTROYS the file if it is re-run after the resolution has been staged, because the
# stages are gone and every side reads as empty (measured this train: a re-run zeroed the
# ledger). Run it once, on an unresolved merge, or restore the stages first.
