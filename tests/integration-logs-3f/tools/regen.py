...
AWK = (r"awk '/^tests\// && !/^tests\/src\/core\/("
       r"ClipSerialisationTest|DataFileSaveIntegrityTest|LufsMeterTest|MidiLearnTest|MixerConcurrencyTest|"
       r"ProjectOpenIntegrityTest|SessionModelTest|StemExportTest|"
       # 3F: the eight fork-NEW tests/src/core sources of the agent control surface, admitted by
       # name in tests/fork-sources.txt's own amended allow-list (see its header, "Merged
       # 2026-09-12 (merge train 3F, unit 1 ...)"). This transcription follows the header.
       r"ControlAutomationScriptTest|ControlDeviceCatalogueTest|ControlEditCommandsTest|ControlRegistryTest|"
       r"ReversibilityContractTest|ReversibilityUndoTest|StableTrackIdsTest"
       r")\.(cpp|h)$/ && !/^tests\/src\/core\/(StemExportTestSupport|ReversibilityTestSupport)\.h$/ && "
       r"!/^tests\/src\/tracks\/SampleClipWindowTest\.cpp$/ { next } { print }'")

# 3F: a third explicit pathspec line admits the thirteen tests/*.py control-surface harnesses by
# name, the same "admitted by name rather than by widening the extension filter" shape the tools/
# line uses. The header's own "Regenerate with" block carries this line too.
TEST_PY = ("tests/agent-surface-gate.py tests/agent_surface_lib.py "
           "tests/control-headless-no-audio-device.py tests/control-headless-project-open.py "
           "tests/control-headless-working-directory.py tests/control-negative-control.py "
           "tests/control-no-audio-device.py tests/control-readiness.py "
           "tests/control-reversibility-transcript.py tests/control-shutdown.py "
           "tests/control_socket_flows.py tests/control_socket_harness.py tests/control-stable-ids.py")
...
def fork_cmd(rev):
    ...
    return (f"{{ {d} -- src include plugins tests/src/core tests/src/tracks "
            ...
            f"{d} -- tools/local-ci.sh tools/ncpu-shim.c plugins/Vst3Instrument/logo.png ; "
            f"{d} -- {TEST_PY} ; }} | LC_ALL=C sort")
