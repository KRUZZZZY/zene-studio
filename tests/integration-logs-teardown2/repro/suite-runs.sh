...
run_suite() { # <tag> <logfile>
  ...
  } | tee -a "$LOGDIR/suite-runs-summary.txt"
}

: > "$LOGDIR/suite-runs-summary.txt"

run_suite "1-alone" "$LOGDIR/suite-run-1-alone.log"
...
run_suite "2-four-parallel-case-loops" "$LOGDIR/suite-run-2-interference-loops.log"
for p in "${pids[@]}"; do kill "$p" 2>/dev/null; done

# interference B: an oversubscribed box (40 busy loops on 20 cores)
hogs=()
for _ in $(seq 1 40); do ( while :; do :; done ) & hogs+=($!); done
run_suite "3-forty-busy-loops" "$LOGDIR/suite-run-3-interference-hogs.log"
for p in "${hogs[@]}"; do kill "$p" 2>/dev/null; done
wait 2>/dev/null

echo "=== suite-runs: DONE ===" | tee -a "$LOGDIR/suite-runs-summary.txt"
