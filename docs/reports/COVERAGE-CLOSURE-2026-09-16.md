...
| 12 | MPE pressure + timbre | `note.expression_set|clear|get` | `ControlNoteExpressionCommandsTest` + `MpePlaybackTest` / `MpeExpressionTest` / `MpeNoteStorageTest`; the audit's "drivable but inert" limit is **lifted** — all three axes reach playback (task #649) | **drivable** (KL:281,462, RN:1087) |
...
| `bash tests/file-length-gate.sh` | **1** | **pre-existing red, not this lane's**: the suite run names `include/ControlRegistryGroups.h (671)`, `include/ControlRegistry.h (509)`, `include/ControlReversibility.h (518)`, `src/core/ControlCommandsNotes.cpp (512)`, `src/core/ControlCommandsProject.cpp (510)`, `src/core/ControlCommandsWarpEdit.cpp (510)`, `src/core/ControlReversibilityTablePassive.cpp (518)`, `tests/control-stable-ids-slice2.py (549)` — every one untouched here, and neither of the two new files (242 / 297 lines) appears |
| `bash tests/run-all-gates.sh` | **1** | `RESULT: FAIL` — gates 3, 6, 8, 9, 10, 11 PASS; **4 (complexity), 5 (mutation), 7 (file-length) FAIL**; 1 (ctest) and 2 (coverage) SKIP. Gates 4 and 7 fail on the pre-existing files listed above; **gate 5 fails only because there is no configured `build/` directory** ("mutation-gate: no build/ — configure first"). Neither new file appears in any regression list. The brief's expected exit 3 (pass-with-skips) is not what this worktree measures, and the difference is not this lane's work |
