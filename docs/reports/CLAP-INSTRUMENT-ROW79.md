...
## 6b. Reproducing this lane's build from scratch (the build dir was deleted, per the brief)

```bash
# the pinned MIT CLAP 1.2.10 headers, copied from a sibling build tree (no network needed)
mkdir -p build && cp -r ../../zene-pa-integration/build-ci/clap build/clap
cmake -S . -B build -DCMAKE_BUILD_TYPE=RelWithDebInfo -DWANT_CLAP=ON -DWANT_VST3=OFF \
      -DWANT_QT6=ON -DWANT_DEBUG_CPACK=OFF -DUSE_COMPILE_CACHE=OFF        # EXIT=0
cmake --build build --target clap-test-instrument clap-test-gain ClapHostTest -j2   # EXIT=0
cd build/tests && ./ClapHostTest && cd ../..                              # 14 passed, 0 failed
```
