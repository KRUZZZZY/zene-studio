...
- **The file-length ratchet is on this lane's radar**: `plugins/ClapEffect/ClapHost.cpp` was 890 lines and the
  baseline (`tests/file-length-baseline.tsv`) holds it at 953; the note path adds ~253 lines to it, so the
  file-length gate will report it. The fix is a mechanical split of the note API + `drainNoteQueue()` into a
  new TU (e.g. `plugins/ClapEffect/ClapNotePath.cpp`, added to the five targets that compile `ClapHost.cpp`),
  **not** a re-anchor.
