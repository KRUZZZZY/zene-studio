...
| 16–28 | `cmake/linux/icons/{same 13 dirs}/mimetypes/application-x-zene-project.png` (13 files) | PNG, 16–256 px | **IDENTICAL** *(at the pre-change revision; the shipped pixels are now the hand-authored placeholder)* | `cmake/linux/icons/{same 13 dirs}/mimetypes/application-x-lmms-project.png` (git rename, pixels unchanged by it) — renamed 2026-09-12 so the raster icon name matches the declared MIME type at every size; see `docs/QDEBUG-CLASS-AND-MIME-RENAME.md` | none | no |
