...
The live transcript of the `export.*` group - its schemas, its A16
`true_inverse` record, the `control.undo` that restores the previous value, the
typed refusals, and the render hashes - is `tests/evidence/export-src-dither/transcript.txt`,
regenerated by `tests/control-export-settings.py`.

The dither test's effect case is the one that matters and it carries its own inverted control: on a
