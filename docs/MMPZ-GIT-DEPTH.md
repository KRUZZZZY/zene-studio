...
* `run-demo.sh` was not re-run end to end in this lane (it needs the binary and
  is the pre-existing demo, not a gate). Its merge sections exercise the same
  paths `depth-demo.sh` now checks, and its `demo_edits.py resolve` still finds
  the new markers because they contain `mmpz-git`.

## 9. Source registration

No new C/C++ sources were added, so nothing was added to
`tests/fork-sources.txt`. That file is regenerated by its own documented rule
(`git diff --diff-filter=A … | grep -E '\.(cpp|c|h|hpp|cc|cxx)$'`) and feeds the
C/C++ analysers (`tests/complexity-gate.sh` via `lizard`,
`tests/file-length-gate.sh`, `tests/duplication-gate.sh`,
`tests/run-coverage.sh`); the new files here are Python and shell developer
tooling under `tools/`, which those gates do not scope. Verified the
file-length gate is unaffected: `bash tests/file-length-gate.sh --check` →
`PASS`, exit 0.
