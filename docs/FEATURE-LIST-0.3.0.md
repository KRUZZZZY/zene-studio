...
| 63 | `automation.record_mode_set` — the record-mode verb the boarded-gaps list names | none yet | **in the tree** — the audit's **to build** is **superseded** (re-measured at `a36e8b0d4`, board task #681): the record-mode verb **is** registered — `automation.record_mode_set` (`src/core/ControlCommandsAutomation.cpp:340`: track / parameter / mode `on|off`, `mutating`, and a result schema reporting `mode_before` and `changed`, so a set is observable) — with its A16 row at `true_inverse` (the clip is a `JournallingObject`, so the inverse is a live checkpoint), and row 10's dependency has landed: the modes work on 2026-09-15 and the refusal the audit read is gone (`automation.mode_set` now sets off / read / touch / latch / write). Proof: the registered ctest `ControlAutomationScriptTest` (the group's contract test — it drives `automation.record_mode_set`), with `AutomationModesTest` and `ControlAutomationModesTest` as the no-destruction property repeated through the command surface; the absence line is one sentence in `docs/KNOWN-LIMITATIONS.md` ("Automation modes are drivable through the socket but not from the interface — added 2026-09-15") | verdict Group A #14; `PLANNED-WORK-MASTER-LIST` :387-390 |
