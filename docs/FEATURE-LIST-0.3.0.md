...
| 58 | The whole-tree ratchet decision | n/a (a decision, executed) | **in the tree** — the decision is recorded in `tests/QA-GATES.md` ("The whole-tree scope's status: ADVISORY SCOPE — the 2026-09-15 decision") and `docs/CONVENTIONS.md`: the whole-tree scope (`tests/all-sources.txt`, gates 4/7/8 `--scope all`) is an **ADVISORY** monitoring ratchet and **not a release gate**; the release gates are the fork and tools scopes, which a bare `run-all-gates.sh` and CI's `static-gates` job run. Measured unpiped on the `030/ratchet-decision` tree at `859883b62`: `complexity-gate.sh --check --scope all` **EXIT=1** (59 regression lines over 42 paths), `file-length-gate.sh --check --scope all` **EXIT=1** (34 over 34), `duplication-gate.sh --scope all` **EXIT=0** (0.64 %), `run-all-gates.sh --whole-tree --no-mutation` **EXIT=1**. The "gates each exit 0" this row used to carry was true of the 2026-09-13 tree only; it is corrected here to the measurement, and every open line was disposed of individually (one `--reanchor-file` per path, each reason naming the class and the growth — never a scope-wide move). `tests/all-sources.txt` now checks itself: `tests/all-sources-reproduce.sh` is its own "Verify it" step and Gate 9 runs it (the manifest had silently lost five more tracked sources, four of them fork-NEW) | charter In §3.2 (Process); `docs/CONVENTIONS.md` |
