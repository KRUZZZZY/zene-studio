...
| 8 | MPE capture, storage and edit (#601) | `note.expression_*` — the same 3 ids as row 7 | **in the tree** — the fields are stored on the `Note` and serialized, and the group is drivable; the audit's *only the pitch axis reaches playback — drivable but inert* is **superseded** by task `#649` (re-measured at `a36e8b0d4`, board task #681): **all three axes reach playback** — pitch as a frequency ratio, pressure and timbre as MIDI events on the note's own member channel — and `docs/KNOWN-LIMITATIONS.md` carries the lifted limit. Proof: the registered ctest `MpeNoteStorageTest` (the per-note fields task `#601` puts on the `Note`, and their serialization), with `MpeExpressionTest` (per-channel capture and pitch math), `MpePlaybackTest` (pressure and timbre reaching the instrument, task `#649`) and `ControlNoteExpressionCommandsTest` (`note.expression_set` / `note.expression_get` / `note.expression_clear` over the surface, each with a `MidiClip` checkpoint as the inverse). One absence line, unchanged: there is still no per-note expression editor | charter In §3.2 (W5); audit §6.2 and Table B #12 |
...
| 63 | `automation.record_mode_set` — the record-mode verb the boarded-gaps list names | none yet | **in the tree** — the audit's **to build** is **superseded** (re-measured at `a36e8b0d4`, board task #681): the record-mode verb **is** registered — `automation.record_mode_set` (`src/core/ControlCommandsAutomation.cpp:340`: track / parameter / mode `on` or `off`, `mutating`, and a result schema reporting `mode_before` and `changed`, so a set is observable) — with its A16 row at `true_inverse` (the clip is a `JournallingObject`, so the inverse is a live checkpoint), and row 10's dependency has landed: the modes work on 2026-09-15 and the refusal the audit read is gone (`automation.mode_set` now sets off / read / touch / latch / write). Proof: the registered ctest `ControlAutomationScriptTest` (the group's contract test — it drives `automation.record_mode_set`), with `AutomationModesTest` and `ControlAutomationModesTest` as the no-destruction property repeated through the command surface; the absence line is one sentence in `docs/KNOWN-LIMITATIONS.md` ("Automation modes are drivable through the socket but not from the interface — added 2026-09-15") | verdict Group A #14; `PLANNED-WORK-MASTER-LIST` :387-390 |
...
| # | Feature | Command group / ids | Status | List it comes from |
|---|---|---|---|---|
| 17 | MIDI learn | `midi.learn_toggle` (of the `midi.*` 2 ids) | **in the tree** — the learn path landed and the id is registered, and the audit's recorded debt (the id appearing **only** in `tests/upstream-modifications.txt`, a manifest) is discharged by a **named proof** (re-measured at `a36e8b0d4`, board task #681): the registered ctest `MidiLearnTest` proves the global MIDI-learn mode headless (a synthetic CC binds the focused control, the binding survives a project round trip, and a learn-off instance ignores the same CC), `MidiLearnThreadTest` covers the arm path across threads, and the registered `ControlSocketIntegration` flow drives `midi.learn_toggle` itself over the wire (armed → disarmed, changed both times, no transaction — it is GUI/engine mode state). The interface half is present, not absent: the Edit > MIDI Learn action declares the command (`src/gui/MainWindow.cpp:384`), which is why the agent-surface baseline does not carry it. The `030/test-gaps` lane's registered reference stands as the earlier half of the same repair | master list ("MIDI learn landed"); audit Table A and §3.3 |
...
| 40 | Autosave / project recovery | `project.restore_revision` (of the `project.*` 4 ids) | **in the tree** — the audit's note that it is **referenced by no behavioural test** is **superseded** (re-measured at `a36e8b0d4`, board task #681): the autosave recovery decision has its own registered ctest, `ProjectRecoveryTest` (offered / refused cases for a fresh, stale, other-project, missing or empty recovery file, and the identity sidecar round trip), `ReversibilityUndoTest` invokes `project.restore_revision` and asserts the typed failure once the rotation has no revision left, and `RevisionTimelineTest` exercises the restore path through the same mechanism `control.undo` dispatches. Limits unchanged, one line each: `revisions.restore` restores the FILE and does not reload the session — `project.open` is how a caller works on the restored bytes — and a live file over the keep-3 policy's per-revision cap is refused, typed, before anything is written | audit Table B #14 and Table A |
...
| 80 | Out-of-process plugin hosting / crash isolation | `oop.*`, 5 ids: `oop.get_state`, `oop.list_families`, `oop.set_mode`, `oop.restart`, `oop.reset_crashes` | **in the tree** — the ZynAddSubFx-only toggle (`post-alpha/oop-hosting`) is generalised into a per-BUILD family table plus the lifecycle record nothing had: which families can be hosted out of process here and the reason for each one that cannot, a typed refusal for every path that is not available, and a crash-loop bound (3 deaths per client executable per session) that `oop.set_mode`/`oop.restart` obey until `oop.reset_crashes` clears it. Proof `OutOfProcessHostTest` (registered ctest). Stated limits: **one** family has an out-of-process path to choose and one more (VST2) is always out of process, so thirteen of the fifteen classified families are refused rather than hosted; no auto-restart; the isolation proof kills the client from outside rather than crashing a plugin by its own bug. The three readings of disagreement 3 below are reconciled in `docs/OUT-OF-PROCESS-BEYOND-ZYN.md` §1 (the table IS the reading: the tree's answer, family by family) | verdict Group A #2; `docs/OOP-HOSTING.md`; `docs/OUT-OF-PROCESS-BEYOND-ZYN.md`; `STATUS-CORRECTION` §93; `ecosystem-gap/` §3.1 |
...
| # | Feature | Command group / ids | Status | List it comes from |
|---|---|---|---|---|
| 47 | The control surface itself: `--control-socket` and the in-app command registry | **33 command groups · 185 command ids** at `3956ef589`, each with schemas and reversibility metadata (34 group names in the registry source, the extra one being the helper-built `wasm` group that no release build compiles in) | **in the tree** — the id set is identical in the registry source and in a live `control.commands_list` (185 = 185, empty difference both ways), and at the audit tip 141 of 150 ids were referenced by a registered test artefact and **149 of 150 were swept to a typed reply** by the registered `agent_surface` ctest (`telemetry.consent` is the one documented allowlist entry). The figures this row carried before (28 groups · 150 ids at the audit tip; 31 groups · 170 ids at the release tree) are superseded. **Named proof (re-measured at `a36e8b0d4`, board task #681):** `ControlRegistryTest` — the in-app command registry's own registered ctest (every required command registered; `describeAll` carrying each id's schemas and `requires`; the typed `not_found`, `invalid_args` and `human` refusals; and the transaction a mutating command still leaves) — with the registered `ControlSocketIntegration` as the socket end-to-end proof, and the registered `agent_surface` ctest named above as the A15 anti-drift leg | charter §3.1 and In §3.2 (A11–A15); audit §1–§3; `docs/COVERAGE-REMEASURE-2026-09-14.md` |
...
| 85 | `telemetry.consent_set` — the consent verb the boarded-gaps list names | none yet | **to build** — the tree registers `telemetry.consent` and `telemetry.status`; `consent_set` is a different id and does not exist at either base. **Closed 2026-09-16 (board task #681): nothing is owed here, and no probe is invented, because there is no command to prove.** The boarded id is refused by recorded decision, not by a gap: `telemetry.consent` **is** the consent verb and declares `requires: display, human`, so no automated caller can consent, and a second agent-reachable consent setter would turn telemetry **on** on the user's behalf, which the feature's design forbids (`docs/TELEMETRY-V1.md` §2.6). The decision is pinned, not merely written — the registered ctest `ControlRegistryTest` carries the telemetry slots that refuse the id's existence, assert that no reachable `telemetry.*` command is mutating, and assert that the only consent verb is `requires: display, human` — and the absence is one line in `docs/KNOWN-LIMITATIONS.md` and in `docs/RELEASE-NOTES-v0.3.0-alpha.md` (both name the boarded verb `consent_set`). What the row measured (the two committed telemetry ids and the absence line) is unchanged, and the row owes no build | verdict Group A #14; `PLANNED-WORK-MASTER-LIST` :387-390 |
...
| # | Feature | Command group / ids | Status | List it comes from |
|---|---|---|---|---|
| 52 | Real-time-safety whole-tree verification programme | n/a (a programme, not a command) | **in the tree** (lane `030/rt-safety`, 2026-09-16, board card #678) — the sweeping half of `AGENTS.md`'s realtime rule ("no allocation, no locking, no unbounded growth on audio-thread paths"), which until now was held per feature (`docs/CONVENTIONS.md` row 9: "partially enforced — a rule held by tests where they exist, not by a sweeping gate"). `tests/rt-safety-sweep.py` sweeps the **27 declared** audio-thread path:symbol pairs in `tests/rt-safety-scope.txt` (the render callback, its four stages, the per-period drains, and the fork's own realtime entries: record demux, retro audio/MIDI rings, the loudness tap, the session scheduler, the MIDI clock, the Lua audio tick) and judges the hits against `tests/rt-safety-allowlist.txt` — a reason **and a count** per accepted hit, a ratchet that may only fall, with `--reanchor "reason"` as its only valve. Measured 2026-09-16: 918 region lines, **4 hits in 3 keys, all in upstream-inherited code** (the render callback's `m_changeMutex` lock, the per-period LFO trigger's `QMutexLocker`, `Song::processNextBuffer`'s two `TrackList` push_backs); every declared fork path is clean; sweep exit **0**. Positive control on the real tree: a deliberate `new` injected into `AudioEngine::renderStageMix` → exit **1**, naming `src/core/AudioEngine.cpp:AudioEngine::renderStageMix:alloc-new`; reverted → exit **0** (both commands in `docs/RT-SAFETY-SWEEP.md`). Proofs: ctests `RtSafetySweep` + `RtSafetySelfTest` (18 controls, incl. the allocation/lock/growth positives and the "outside the declared scope is not seen" bound control), Gate 12 in `tests/run-all-gates.sh`, a step in CI's `static-gates` job. **Stated bound**: static (the runtime half stays the `AllocationProbe` tests), scope declared not discovered, no virtual dispatch/macros, no cost/syscall/aliasing analysis, render thread only — printed by every run and written out in `docs/RT-SAFETY-SWEEP.md`. No command group: a programme, not a command (this row's own schema), so there is no UI absence to declare — the absence item this row carried is moot | charter In §3.2 (verification programmes); ledger, absent item 2; audit §6.1; `docs/RT-SAFETY-SWEEP.md`; `tests/QA-GATES.md` Gate 12 |
...
| 56 | `REPO-2` — a gate refusing evidence file types and oversized files | n/a (a gate) | **in the tree** — `tests/evidence-gate.sh` (Gate 11) landed 2026-09-13 (`a867fd493`, the same series that executed `CP-1`'s deletion arm as `REPO-1`) and is wired into `run-all-gates.sh`'s gate 11 row and CI's `static-gates` job. It refuses four classes **by name** anywhere in the tree — evidence suffixes (`log exit ours theirs` plus coverage, profile and machine-readable-report data), renders outside `data/`, archives and packages, compiled and dependency artefacts — plus anything over `EVIDENCE_SIZE_CAP_BYTES` (1 MiB), with a required reason-bearing exemption list (`tests/evidence-gate-exempt.txt`; blank reason and a missing file are both exit 2) and recorded exit codes (0 clean / 1 refused / 2 setup error). Extended 2026-09-16 (`030/repo2-gate`, board card #683) to close this row's own wording ("a binary/audio/archive extension list plus a size ceiling"): the archive and compiled-artefact classes were missing, so a committed `.zip` or `.o` was caught only by the cap — which compression defeats by construction. Its red/green control now runs in the same gate row and as its own CI step: 9 fixtures / 13 verdicts, four of them asserting the refused path is **named** in the output, one of them a **tracked** over-cap file through the git index (the path the suite actually takes, previously unexercised). First live catch: the wave-9 `docs/reports/VST3-INSTRUMENT-ROW78-EVIDENCE.log`, which was simultaneously the release line's only Gate 6 violation; resolved 2026-09-16 on `CP-1`'s terms — file deleted, its sha256 appended to `tests/evidence-manifest.tsv` (now 1,396 entries) — so Gate 6 and Gate 11 are green on the result **by removal**, with no gate re-anchored and no accepted-violation row added. The 22 `docs/release-verification-0.2.0-alpha/*.log` violations this row was blocked on were already gone (moved out of `docs/` by `3fe5addb9`, deleted by `REPO-1`), re-measured here: 0 remain. Recorded open item: `plugins/RnnoiseDenoiser/testdata/` is exempt with a stated reason (its `crash-evidence/*.log` and `strace_A.log` are run output no script reads; the owner's `CP-1` deletion set did not name that directory) — the exemption file says so in as many words. | charter In §3.2 (Process); master list `REPO-2` |
| 57 | `REPO-4` — move lane reports and transcripts out of the repository root | n/a (a move) | **in the tree** — done in two passes, both recorded in `docs/reports/README.md`: the 2026-09-13 pass moved `DOCS-NAMING.md` and the `CMDN-*-REPORT`/`-TRANSCRIPT` pair; the 2026-09-15 pass (`030/repo4-hygiene`, merged here) moved the seven `LANE-STATE*.md` reports to `docs/reports/`, so the repository root holds exactly `README.md`, `CONTRIBUTING.md`, `SECURITY.md` and the build files (`ls -1 *.md`), and the seven reports carry a dated Location note at their head. The rule — a lane's report and its transcript go to `docs/reports/`, the root is not a candidate — is written in `docs/reports/README.md` and in `docs/CONVENTIONS.md`, which also records that **no gate enforces it, deliberately**: the check would have to guess which root `.md` is a lane report, and a guess is what gets weakened | charter In §3.2 (Process); master list `REPO-4` |
| 58 | The whole-tree ratchet decision | n/a (a decision, executed) | **in the tree** — the decision is recorded in `tests/QA-GATES.md` ("The whole-tree scope's status: ADVISORY SCOPE — the 2026-09-15 decision") and `docs/CONVENTIONS.md`: the whole-tree scope (`tests/all-sources.txt`, gates 4/7/8 `--scope all`) is an **ADVISORY** monitoring ratchet and **not a release gate**; the release gates are the fork and tools scopes, which a bare `run-all-gates.sh` and CI's `static-gates` job run. Re-measured on `030/ratchet-decision` at `a6e1b62ae` (2026-09-16): `complexity-gate.sh --check --scope all` **EXIT=1** (49 regression lines over 37 paths), `file-length-gate.sh --check --scope all` **EXIT=1** (23 over 23), `duplication-gate.sh --scope all` **EXIT=0** (0.64 %). Every open line was then disposed of **individually**: `tests/control-detect-commands.py`'s `run_checks` (CCN 75 — the worst fork-authored function in the tree) is **fixed by splitting** and needs no baseline entry in either scope, and the other 60 paths are grandfathered by **60 single-path `--reanchor-file` records**, each reason naming the path, its class and the measured growth and quoted verbatim in `tests/QA-GATES.md` — never a scope-wide move. After them the whole-tree scope is **complexity EXIT=0, file-length EXIT=0, duplication EXIT=0** and `run-all-gates.sh --whole-tree --no-mutation` exits **3** (pass-with-skips: gates 1/2/5 need a build), the pre-tag shape. The manifest behind the scope checks itself: `tests/all-sources-reproduce.sh` is `all-sources.txt`'s own recipe run as a check, Gate 9 runs it every run (**EXIT=0**, and `--self-test` proves the control refuses a dropped and an underivable path), and the same check found the sibling `tests/fork-sources.txt` unable to derive ten of its own entries — fixed by a pathspec in its recipe, with nothing untoward registered | charter In §3.2 (Process); `docs/CONVENTIONS.md` |
...
| 15 | Real-time-safety whole-tree verification programme | ~~none~~ `tests/rt-safety-sweep.py` (ctest `RtSafetySweep`), `tests/rt_safety_selftest.py` (ctest `RtSafetySelfTest`, 18 controls), scope + allowlist ledgers, Gate 12 | **no longer absent** — in the tree since 2026-09-16 (lane `030/rt-safety`, board card #678), measured exit 0 with the four accepted hits all in inherited code and the positive control recorded in `docs/RT-SAFETY-SWEEP.md`; see the row-52 entry above. The absence line the release documents would have carried for this item is moot | 52 |
