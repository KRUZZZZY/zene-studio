...
Environment detail: `tests/coverage-run/environment.txt`.
...
`tests/coverage-run/classify-unseen.py`; full output: `unseen-classification.txt`):
...
All evidence is committed on `post-alpha/coverage-run` under **`tests/coverage-run/`**. Nothing
was parked in `/tmp` (the reason this requirement exists is the earlier lane whose `/tmp`
evidence a disk reclaim destroyed).

*(Merge train 3A, 2026-09-12. This directory was `docs/coverage-run/` on the lane's own branch and was
relocated to `tests/coverage-run/` at integration: Gate 6 (`tests/no-upstream-regression-gate.sh`)
classifies a non-`.md` path under `docs/` as an undeclared upstream divergence and reds the gate on all
14 files. `tests/**` is the category Gate 6 allows, and it is where this repo already keeps its durable
evidence (`tests/reference/`, `tests/scripted/`, `tests/gate-hygiene-logs/`). Only the paths changed -
no measurement, log or number in this report was altered, and the referenced scripts were edited for the
path alone.)*
...
tracefile with one `genhtml tests/coverage-run/coverage-fork.info`; the uncompressed 15.7 MB
...
bash tests/coverage-run/gate-controls.sh . tests/coverage-run/coverage-fork.info
```
