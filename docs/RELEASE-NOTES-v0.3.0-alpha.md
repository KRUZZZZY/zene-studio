...
- **UI absence — one line per group:** PDC is drivable through the socket and nothing in the interface shows a
  ...
  nothing in the interface draws a patch bay, a cable, a node or a port, and no command edited a graph at
  all until `patcher.set_wiring` (feature row 69, the section below);
