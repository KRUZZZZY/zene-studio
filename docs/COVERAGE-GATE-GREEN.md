...
## Evidence relocated to `tests/coverage-green/` at integration (merge train 3B)

Gate 6 (`tests/no-upstream-regression-gate.sh`) classifies a non-`.md` path under `docs/` as an
undeclared upstream divergence, and it reds the gate on all 16 non-`.md` files this lane's evidence
originally sat under `docs/coverage-green/`. Measured at integration, before the merge commit:
16 VIOLATIONs, every one of them a file in that directory (`.py`, `.sh`, `.log`, `.gz`, `.txt`,
`.tsv`). `tests/**` is the category Gate 6 allows, and it is where this repo already keeps its
durable evidence (`tests/reference/`, `tests/scripted/`, `tests/coverage-run/` from the identical
finding against `post-alpha/coverage-run` in train 3A). The directory was therefore relocated to
`tests/coverage-green/` in the merge commit, without weakening any gate and without editing a gate
script. Only the paths changed - no measurement, log or number was altered, and the referenced
scripts were edited for the path alone.

The alternative - teaching Gate 6 a `docs/` category on the argument that a fork-authored evidence
directory under `docs/` is not an upstream divergence - was considered and refused for the same
reason 3A refused it: the relocation makes the path legal without relaxing a gate, while the
category change relaxes a gate. **See "Decisions needed" in docs/INTEGRATION-MERGES-3B.md.**
---
...
`EXIT=139`. `tests/coverage-green/midilearngui-segfault-backtrace.log`:
...
**(e) What is actually unhit** — `tests/coverage-green/unhit-lines.txt`, generated by
...
The three reasons are printed in full in `tests/coverage-green/reanchor-migration.log`. Baseline diff:
...
Breakdown from `tests/coverage-green/classify-scope.py` (output:
`tests/coverage-green/scope-classification.txt`), each entry matched against the objects the build
actually produced:
...
$ bash tests/coverage-green/gate-controls.sh . build-coverage/coverage/coverage-fork.info
...
`tests/coverage-green/gate-controls.log`.
...
| `build-coverage-logs/` (raw logs; the committed extracts live in `tests/coverage-green/`) | 45 MB |
...
(`tests/coverage-green/run-coverage.log.gz`, 2.4 MB) — nothing was parked in `/tmp`.
...
* **`tests/test-verification-debt.sh`** exercises the changed gate on synthetic fixtures; it is
  green (exit 0, all assertions held — `tests/coverage-green/verification-debt.log`).

---

## 9. Evidence inventory — `tests/coverage-green/`
...
bash tests/coverage-green/gate-controls.sh . build-coverage/coverage/coverage-fork.info
python3 tests/coverage-green/classify-scope.py build-coverage/coverage/coverage-fork.info build-coverage
```
