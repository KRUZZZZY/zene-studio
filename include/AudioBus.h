...
public:
	...
	AudioBus(SampleFrame* const* bus, ch_cnt_t channelPairs, f_cnt_t frames)
		: m_bus{bus}
	...
	auto trackChannelPair(ch_cnt_t pairIndex) const -> InterleavedBufferView<const float, 2>
	{
		return {m_bus[pairIndex], m_frames};
	}

	auto trackChannelPair(ch_cnt_t pairIndex) -> InterleavedBufferView<float, 2>
	{
		return {m_bus[pairIndex], m_frames};
	}

	//! @return 2-channel interleaved buffer for the given track channel pair
	auto operator[](ch_cnt_t pairIndex) const -> const float*
	{
		return reinterpret_cast<const float*>(m_bus[pairIndex]);
	}

	//! @return 2-channel interleaved buffer for the given track channel pair
	auto operator[](ch_cnt_t pairIndex) -> float*
	{
	...
	auto channels() const -> ch_cnt_t { return m_channelPairs * 2; }
	auto channelPairs() const -> ch_cnt_t { return m_channelPairs; }
...
private:
	 ...
	 */
	auto update(const std::bitset<MaxTrackChannels>& channels, ch_cnt_t upperBound = MaxTrackChannels) -> bool;

	SampleFrame* const* m_bus = nullptr; //!< [channel pair index][sample index]
	const ch_cnt_t m_channelPairs = 0;
